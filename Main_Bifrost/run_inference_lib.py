...
def run_inference(temp_dir_path, bg_image_path, bg_mask, ref_object_location, 
    ...
    """
    Run inference to generate a new image with the reference object placed in the background.
    ...
    Args:
        ...
        depth (list): Depth scale range [min, max]
        pixel_num (float): Number of pixels for mask augmentation
        mode (str): Fusion mode ('place', 'replace', 'draw')
    ...
    Returns:
        numpy.ndarray: Generated image with reference object placed in background
    """
    if bg_mask[0]+bg_mask[2] > 1 or bg_mask[1]+bg_mask[3] > 1:
        raise ValueError('The mask is out of range')
    ...
    # Get reference image path from background image path
    ref_image_path = bg_image_path.replace('background.jpg', 'object.jpg')
    ...
    # Generate mask for reference image using SAM
    ...
    # Generate mask for background image using SAM
    ...
    # Get depth maps using DPT
    input_folder = os.path.dirname(bg_image_path)
    run(dpt_model, transform, input_folder, output_depth_folder)

    # Process masks and depth maps
...
if __name__ == '__main__':
    ...
    gen_image = run_inference(temp_dir_path, bg_image_path, bg_mask, ref_object_location)
